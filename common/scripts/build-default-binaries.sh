...
do
    ...
    echo "---------------------------------------------------------------"
    this_cmd="icpx -fsycl -fintelfpga -Xshardware -Xstarget="$ASP_ROOT":"$this_variant" -Xsclock=200MHz -Xsbsp-flow="$ASP_FLOW" "$ASP_ROOT/$CPP_FILE" -DFPGA_HARDWARE -o "$this_variant".fpga"
